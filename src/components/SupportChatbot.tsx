    ...
    <div className="relative font-['Inter',sans-serif]">
      ...
      {!open && (
        <button
          ...
          className="fixed bottom-5 right-5 z-40 bg-[#006a39] hover:bg-[#00522c] text-white py-2.5 px-4 rounded-full shadow-lg shadow-emerald-950/20 flex items-center gap-2.5 transition-all duration-200 hover:scale-105 active:scale-95 cursor-pointer border-2 border-white/80 group"
          aria-label="Open 24/7 SubhOne Support Chat"
        >
          <div className="relative flex items-center justify-center">
            <span className="text-lg">💬</span>
            <span className="absolute -top-0.5 -right-0.5 w-2.5 h-2.5 bg-emerald-400 border-2 border-[#006a39] rounded-full animate-ping" />
            <span className="absolute -top-0.5 -right-0.5 w-2.5 h-2.5 bg-emerald-400 border-2 border-[#006a39] rounded-full" />
          </div>
          <div className="text-left hidden sm:block">
            <p className="text-xs font-black tracking-wide leading-tight">24/7 Support</p>
            <p className="text-[10px] text-emerald-200 font-semibold leading-none mt-0.5">Online Care</p>
          </div>
        </button>
      )}
      ...
      {open && (
        <div className="fixed bottom-4 right-4 sm:bottom-6 sm:right-6 z-50 w-[calc(100vw-32px)] sm:w-96 bg-white/95 backdrop-blur-xl rounded-3xl shadow-2xl border border-[#d6e4d8] overflow-hidden flex flex-col max-h-[80vh] h-[520px] animate-in slide-in-from-bottom-5 duration-200">
