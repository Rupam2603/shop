  ...
  try {
    ...
    const res = await fetch('/api/create-order', {
      ...
      body: JSON.stringify({
        ...
        paymentType: params.userRole === 'retailer' ? 'COD' : (params.paymentMethod === 'Card' || params.paymentMethod === 'UPI' ? 'online' : params.paymentMethod),
        paymentRef: null,
    ...
    const fullOrder: DbOrder = {
      ...
      payment_method: data.payment_method || (params.userRole === 'retailer' ? 'COD' : params.paymentMethod) || "COD",
      payment_status: data.payment_status || (params.userRole === 'retailer' ? 'Pending' : ((params.paymentMethod === 'Card' || params.paymentMethod === 'UPI') ? "Paid" : "Pending")),
